...
use_papyrus = os.environ.get("USE_PAPYRUS", "False") == "True"
...
SYSTEM_PROMPT = """
You are Lumio, a highly capable AI assistant designed to handle any task presented by the user. You have access to a variety of tools that you can invoke to efficiently complete complex objectives.
        ...
        while True:
            try:
                ...
                if not instruction:
                    continue
                instruction = (
                    instruction[1:] if instruction.startswith("/") else instruction
                )
                if instruction.lower() in ["exit", "quit"]:
                    print("Exiting Lumio CLI. Goodbye!")
                    break
                elif instruction.lower() == "help":
                    print("Available commands: repo, do, build, help, exit, quit")
                else:
                    args = shlex.split(instruction)
                    fire.Fire(self, args)
            except KeyboardInterrupt:
                print("\nExiting Lumio CLI. Goodbye!")
                break

    def repo(self, instruction: str, **kwargs):
        asyncio.run(self.code_lumio.execute(instruction, **kwargs))

    def do(self, instruction: str, **kwargs):
        asyncio.run(self.do_lumio.execute(instruction, **kwargs))

    def build(self, **kwargs):
        asyncio.run(
            self.code_lumio.execute(
                instruction="""
